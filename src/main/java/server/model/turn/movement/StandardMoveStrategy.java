...
public class StandardMoveStrategy implements Movement {
    ...
    protected ArrayList<Action> makeMoveActions(Worker worker, Point from, Map<Point, Action> tosAndForcedActionsMap) {
        ...
        tosAndForcedActionsMap.keySet().forEach(to -> actions.add(new Action(worker.getReadableCopy(), Action.ActionType.MOVE, from, to, tosAndForcedActionsMap.get(to), null)));
        return actions;
    }
