    ...
    private static class TimerThread implements Runnable {
        ...
        public void run() {
            while(running) {
                if(timeLeft > 0) {
                    if(timeLeft % 20 == 0)
                        view.printLiveTimerMessage(CliConstants.TIME_LEFT + " " + timeLeft / 60 + CliConstants.MINUTES + " " + timeLeft % 60 + CliConstants.SECONDS);
                    try {
                        Thread.sleep(1000);
                    ...
                    lockable.interrupt();
                    view.printLiveTimerMessage(CliConstants.TIME_EXPIRED);
                    running = false;
                }
            }
        }
